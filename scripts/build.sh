...
This script builds the drupalhub application.
...
# Chores.
(
  for DIR in $BUILD_ROOT/www-build sites-backup drupalhub/1 drupalhub/modules/contrib drupalhub/themes/contrib drupalhub/libraries; do
    rm -Rf $DIR
  done
)
...
(
  # Build core.
  $DRUSH make drupalhub/drupal-org-core.make $BUILD_ROOT/www-build
  ...
  ln -s ../../drupalhub $BUILD_ROOT/www-build/profiles/drupalhub
...
(
  if [ -n "$OPSDIR" ] && [ -d $BUILD_ROOT/$DOCROOT/$OPSDIR ] ; then
    ...
    $DRUSH make --no-core --contrib-destination $CUR_DIR/$BUILD_ROOT/drupalhub/os-ops.make .
    cd $CUR_DIR
  fi
)
